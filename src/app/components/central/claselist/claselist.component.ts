...
@Component({
  selector: 'app-claselist',
  templateUrl: './claselist.component.html',
  styleUrls: ['./claselist.component.css']
})
 ...
 */
   eliminar(id){
    ...
    Json.estadoClase=2;
    console.log(Json);
    this.data.estadoClaseVehiculo(Json).subscribe(
      (res:any)=>{
  ...
  filtrar()
  {
    this.data.filtrarClaseVehiculo(this.criterioFiltro,this.valorFiltro).then(respuesta =>
      {
        console.log(respuesta)
        this.res = respuesta.clases;
      }).catch(error =>
        {
